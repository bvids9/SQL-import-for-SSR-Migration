...
This is a fork of [ArasLabs/SQL-import-for-SSR-Migration](https://github.com/ArasLabs/SQL-import-for-SSR-Migration).

Changes from the original:
- Refactored method code into a testable C# class (`SqlToReportConverter`)
- Added `ILogger` interface with `ConsoleLogger` for local testing and `ArasLogger` (CCO) for Aras
- Standardised return types across methods (`string` for error propagation, `void` where no return is needed)
- Added XML documentation comments
- Added a `Development/` folder with a dotnet-script testbed (`.csx`) for testing SQL parsing without a live Aras instance
...
A dotnet-script testbed is available in the `Testing/` folder for testing SQL parsing without a live Aras instance. This covers the parsing half of the tool — `WriteQueryDefinition` and `WriteReportDefinition` require a live Aras connection and are not included in the testbed.
...
```
cd Testing
dotnet script SqlToReport.csx
```

### Example SQL

The testbed accepts standard T-SQL SELECT statements targeting Aras Innovator's underlying SQL Server tables. For example:
...
FROM
    innovator.part p
    LEFT JOIN innovator.user u ON u.id = p.created_by_id
WHERE
    p.state = 'Released'
```
...
Original project created by Aras Development and AJ Sebastian at Aras Labs. [@asebastian-aras](https://github.com/asebastian-aras)
